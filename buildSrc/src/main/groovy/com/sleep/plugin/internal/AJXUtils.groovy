 ...
 */
package com.sleep.plugin.internal
...
import com.sleep.plugin.FileLog
import com.sleep.plugin.AopPlugin
...
class AJXUtils {
    ...
    static <T> T optFromJsonString(String jsonString, Class<T> clazz) {
        try {
            ...
            LoggerFactory.getLogger(AopPlugin).warn("optFromJsonString(${jsonString}, ${clazz}", e)
        }
        return null
    }
    ...
    static <T> T optFromJsonString(String json, Type typeOfT) {
        try {
            ...
            LoggerFactory.getLogger(AopPlugin).warn("optFromJsonString(${json}, ${typeOfT}", var3)
        }
        return null
    }
    ...
    static String optToJsonString(Object object) {
        try {
            ...
            LoggerFactory.getLogger(AopPlugin).warn("optToJsonString(${object}", var2)
        }
        return null
    }
    ...
    static void doWorkWithNoAspectj(TransformInvocation transformInvocation) {
        LoggerFactory.getLogger(AopPlugin).debug("do nothing ~~~~~~~~~~~~~~~~~~~~~~~~")
        ...
        transformInvocation.inputs.each { TransformInput input ->
            ...
            input.jarInputs.each { JarInput jarInput->
                def dest = transformInvocation.outputProvider.getContentLocation(jarInput.name
                        ...
                        , jarInput.scopes
                        , Format.JAR)
                FileLog.logI("copy file, input = ${jarInput.file.absolutePath}, dest = $dest")
    ...
    static void mergeJar(File sourceDir, File targetJar) {
        ...
        try {
            ...
            LoggerFactory.getLogger(AopPlugin).warn("mergeJar(${sourceDir}, ${targetJar}", e)
        } finally {
            jarMerger.close()
        }
    }
