...
import com.sleep.plugin.internal.cache.AJXCache
import com.sleep.plugin.internal.cache.VariantCache
import com.sleep.plugin.internal.model.AJXExtensionConfig
...
abstract class AbsProcedure {
    ...
    boolean doWorkContinuously() {
        for (AbsProcedure procedure : procedures) {
            boolean result = procedure.doWorkContinuously()
            FileLog.logI("${procedure.class.getSimpleName()} work done, result = ${result}")
            if (!result) {
                break
            }
        }
